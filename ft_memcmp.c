...
/*   Updated: 2024/09/23 17:02:29 by cdaureo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	size_t i;
	unsigned char *s1temp = (unsigned char *) s1;
	unsigned char *s2temp = (unsigned char *)s2;
...
int main(void) {
    ...
    if (result == 0) {
        printf("%d Las dos cadenas son iguales en los primeros 12 bytes.\n",result);
    } else if (result > 0) {
